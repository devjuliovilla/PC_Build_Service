...
@Component({
  ...
  template: `
    <jv-page [title]="pageTitle()" [description]="description()">
      <div page-actions>
        <jv-icon-button
...
export default class CategoryPage implements OnDestroy {
  ...
  protected pageTitle = computed(() => {
    ...
    return cat ? `Componentes: ${cat}` : 'Todos los Componentes';
  });

  protected description = computed(() => {
    const cat = this.category();
    return cat
      ? `Componentes de la categoría "${cat}" — ${this.components().length} productos`
      : `Catálogo completo — ${this.components().length} productos`;
  });

  protected visibleCards = computed(() => {
    const start = this.cardPageIndex() * this.cardPageSize();
    return this.components().slice(start, start + this.cardPageSize());
  });

  protected gridColumns: JvGridColumn<ComponentModel>[] = [
  ...
  private loadComponents() {
    const cat = this.category();
    const obs = cat
      ? this.componentsService.byCategory(cat)
  ...
  protected onRowClick(component: ComponentModel) {
